...
export default function Page() {
  ...
  // Login with ID + password via Supabase users table
  const doLogin=async()=>{
    setLoginErr("");
  ...
  if(!user)return<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(160deg,#0f172a,#1e293b,#0f172a)",fontFamily:"Inter,'Noto Sans SC',sans-serif"}}>
    ...
    <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,padding:"36px 32px",textAlign:"center",maxWidth:360,width:"100%"}}>
      ...
      <input value={loginId} onChange={e=>{setLoginId(e.target.value);setLoginErr("")}} placeholder="账号 / User ID" style={{width:"100%",padding:"9px 12px",borderRadius:8,border:"1px solid rgba(255,255,255,0.15)",background:"rgba(255,255,255,0.06)",color:"#f8fafc",fontSize:14,outline:"none",textAlign:"center",marginBottom:10,fontFamily:"inherit",boxSizing:"border-box"}}/>
      <input value={loginPw} onChange={e=>{setLoginPw(e.target.value);setLoginErr("")}} onKeyDown={e=>e.key==="Enter"&&doLogin()} placeholder="密码 / Password" type="password" style={{width:"100%",padding:"9px 12px",borderRadius:8,border:"1px solid rgba(255,255,255,0.15)",background:"rgba(255,255,255,0.06)",color:"#f8fafc",fontSize:14,outline:"none",textAlign:"center",marginBottom:10,fontFamily:"inherit",boxSizing:"border-box"}}/>
      {loginErr&&<div style={{color:"#f87171",fontSize:11,marginBottom:8}}>{loginErr}</div>}
      <button onClick={doLogin} style={{width:"100%",padding:"9px",borderRadius:8,border:"none",background:"#6366f1",color:"white",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>登录 / Log In</button>
    </div>
  </div>;
  ...
  return<div style={{fontFamily:"Inter,'Noto Sans SC',sans-serif",background:"linear-gradient(160deg,#0f172a,#1e293b,#0f172a)",minHeight:"100vh",color:"#e2e8f0"}}>
    ...
    <div style={{padding:"16px 22px",maxWidth:1100}}>
      ...
      {/* OVERVIEW */}
      {tab==="overview"&&<div>
        <h2 style={{fontSize:13,fontWeight:700,marginBottom:10,color:"#cbd5e1"}}>项目构成 / Project Components</h2>
        ...
        <h2 style={{fontSize:13,fontWeight:700,marginBottom:10,color:"#cbd5e1"}}>项目周期 / Project Lifecycle</h2>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:7}}>
          <h2 style={{fontSize:13,fontWeight:700,color:"#cbd5e1",margin:0}}>技术参数 / Technical Parameters</h2>
          <button onClick={addP} style={{background:"none",border:"1px solid rgba(99,102,241,0.3)",borderRadius:5,color:"#818cf8",padding:"2px 7px",cursor:"pointer",fontSize:10,fontFamily:"inherit"}}>+ 添加 / Add</button>
        </div>
      ...
      {/* BLOCKERS */}
      {tab==="blockers"&&<div>
        <div style={{fontSize:11,color:"#94a3b8",marginBottom:12,padding:"6px 10px",background:"rgba(99,102,241,0.08)",borderRadius:7}}>💡 点击文字编辑 / Click text to edit · 下拉切换 / Dropdown to change · ✕ 删除 / Delete · + 新增 / Add new</div>
        <h3 style={{fontSize:12.5,fontWeight:700,color:"#f87171",marginBottom:7}}>技术层面 / Technical</h3>
        ...
        <h3 style={{fontSize:12.5,fontWeight:700,color:"#fbbf24",marginBottom:7,marginTop:14}}>商务层面 / Commercial</h3>
        ...
        <h3 style={{fontSize:12.5,fontWeight:700,color:"#38bdf8",marginBottom:7,marginTop:14}}>许可合规 / Permits & Compliance</h3>
      ...
      {/* TIMELINE */}
      {tab==="timeline"&&<div>
        <div style={{fontSize:11,color:"#94a3b8",marginBottom:12,padding:"6px 10px",background:"rgba(99,102,241,0.08)",borderRadius:7}}>💡 拖动滑块更新进度 / Drag slider · 点击文字编辑 / Click to edit · ✕ 删除 / Delete · + 新增 / Add</div>
          ...
          {data.timeline.map((t,i)=><div key={t.id} style={{position:"relative",marginBottom:16,paddingLeft:22}}>
            ...
            <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:9,padding:11}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4,flexWrap:"wrap",gap:3}}>
                <span style={{fontSize:12.5,fontWeight:700,color:"#e2e8f0",flex:1}}><EC value={t.phase} ck={`tl-${t.id}-p`} onC={v=>up(d=>{d.timeline[i].phase=v},`阶段 Phase: ${v.slice(0,18)}`)}/></span>
                <div style={{display:"flex",alignItems:"center",gap:5}}>
                  <span style={{fontSize:10.5,color:"#818cf8",fontFamily:"'JetBrains Mono',monospace"}}><EC value={t.duration} ck={`tl-${t.id}-d`} onC={v=>up(d=>{d.timeline[i].duration=v},`工期 Duration: ${v}`)}/></span>
                  <Del onClick={()=>rmTL(i)}/>
                </div>
              </div>
      ...
      {/* ACTIONS */}
      {tab==="actions"&&<div>
        <h3 style={{fontSize:12.5,fontWeight:700,color:"#f87171",marginBottom:7}}><span style={{background:"#ef4444",color:"white",fontSize:9,padding:"2px 6px",borderRadius:9,marginRight:6}}>P0</span>立即启动 / Immediate</h3>
        {data.p0Actions.map((a,i)=><div key={a.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 11px",marginBottom:4,background:a.done?"rgba(34,197,94,0.06)":"rgba(255,255,255,0.03)",border:`1px solid ${a.done?"rgba(34,197,94,0.2)":"rgba(255,255,255,0.08)"}`,borderRadius:7}}>
          ...
          <span style={{flex:1,fontSize:12,color:a.done?"#64748b":"#e2e8f0",textDecoration:a.done?"line-through":"none"}}><EC value={a.text} ck={`a-${a.id}`} onC={v=>up(d=>{d.p0Actions[i].text=v},`编辑 Edit P0: ${v.slice(0,20)}`)}/></span>
          <Del onClick={()=>rmA("p0Actions",i)}/>
        </div>)}<Add onClick={()=>addA("p0Actions","P0")} label="P0 行动项 / P0 Action"/>
        <h3 style={{fontSize:12.5,fontWeight:700,color:"#fbbf24",marginBottom:7,marginTop:16}}><span style={{background:"#ca8a04",color:"white",fontSize:9,padding:"2px 6px",borderRadius:9,marginRight:6}}>P1</span>近期启动 / Near-Term</h3>
        {data.p1Actions.map((a,i)=><div key={a.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 11px",marginBottom:4,background:a.done?"rgba(34,197,94,0.06)":"rgba(255,255,255,0.03)",border:`1px solid ${a.done?"rgba(34,197,94,0.2)":"rgba(255,255,255,0.08)"}`,borderRadius:7}}>
          ...
          <span style={{flex:1,fontSize:12,color:a.done?"#64748b":"#e2e8f0",textDecoration:a.done?"line-through":"none"}}><EC value={a.text} ck={`a-${a.id}`} onC={v=>up(d=>{d.p1Actions[i].text=v},`编辑 Edit P1: ${v.slice(0,20)}`)}/></span>
      ...
      {tab==="risks"&&<div>
        {data.risks.map((r,i)=>{const cc={"技术 / Technical":"#818cf8","融资 / Financing":"#fbbf24","政策 / Policy":"#f87171","物流 / Logistics":"#38bdf8","合规 / Compliance":"#a78bfa"};return<div key={r.id} style={{display:"flex",gap:9,padding:"10px 12px",marginBottom:5,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:9,alignItems:"center",flexWrap:"wrap"}}>
          <select value={r.category} onChange={e=>up(d=>{d.risks[i].category=e.target.value},`类别 Category → ${e.target.value}`)} style={{fontSize:10,padding:"2px 5px",borderRadius:12,fontWeight:700,border:"none",background:`${cc[r.category]||"#818cf8"}22`,color:cc[r.category]||"#818cf8",cursor:"pointer"}}>{RC.map(c=><option key={c} value={c}>{c}</option>)}</select>
  ...
  {user==="Admin"&&<button onClick={async()=>{const{error}=await supabase.from("changelog").delete().eq("id",l.id);if(!error){setLogs(prev=>prev.filter(x=>x.id!==l.id));flash("已删除 Deleted ✓")}}} style={{background:"none",border:"none",color:"#475569",cursor:"pointer",fontSize:11,padding:"0 2px",opacity:.4}} onMouseOver={e=>{e.currentTarget.style.opacity=1;e.currentTarget.style.color="#ef4444"}} onMouseOut={e=>{e.currentTarget.style.opacity=.4;e.currentTarget.style.color="#475569"}}>✕</button>}
      ...
      {/* HELP / GUIDE */}
      {tab==="help"&&<div>
        <h2 style={{fontSize:15,fontWeight:700,color:"#cbd5e1",marginBottom:14}}>使用说明 / User Guide</h2>
        {[
          ...
          {title:"风险 Risks",icon:"△",desc:"记录和管理项目风险。用下拉菜单选择风险类别（技术/融资/政策/物流/合规）；点击风险描述和缓解措施可编辑；✕ 删除；+ 新增。/ Record and manage risks. Dropdown to select category (Technical/Financing/Policy/Logistics/Compliance); click risk or mitigation to edit; ✕ to delete; + to add."},
          {title:"变更记录 Changelog",icon:"◷",desc:"自动记录所有团队成员的操作，包括谁在什么时间做了什么修改。不同成员用不同颜色标识，便于区分。记录自动保存，无法手动清除。/ Automatically logs all team actions: who changed what and when. Different members shown in different colors. Records auto-saved and cannot be manually cleared."},
        ].map((h,i)=><div key={i} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,padding:"14px 16px",marginBottom:10}}>
          <div style={{fontSize:13,fontWeight:700,color:"#c7d2fe",marginBottom:6}}><span style={{marginRight:6}}>{h.icon}</span>{h.title}</div>
          <div style={{fontSize:11.5,color:"#94a3b8",lineHeight:1.6}}>{h.desc}</div>
        </div>)}
        <div style={{background:"rgba(99,102,241,0.08)",border:"1px solid rgba(99,102,241,0.15)",borderRadius:10,padding:"14px 16px",marginTop:14}}>
          <div style={{fontSize:13,fontWeight:700,color:"#818cf8",marginBottom:6}}>🔐 账号管理 / Account Management</div>
          <div style={{fontSize:11.5,color:"#94a3b8",lineHeight:1.6}}>账号由管理员在 Supabase 数据库中创建和管理。如需新增账号或重置密码，请联系项目管理员。/ Accounts are created and managed by the admin in the Supabase database. Contact the project admin to add accounts or reset passwords.</div>
        </div>
      </div>}
    </div>
  </div>;
